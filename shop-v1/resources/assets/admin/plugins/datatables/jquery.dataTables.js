...
(function( $, window, document, undefined ) {
		...
		if ( ! DataTable.__browser ) {
			...
			var n = jQuery('<div/>')
				.css( {
					position: 'fixed',
					top: 0,
					left: jQuery(window).scrollLeft()*-1, // allow for scrolling
					height: 1,
					width: 1,
					overflow: 'hidden'
				} )
	...
	function _fnVisbleColumns( oSettings )
	{
		...
		$.each( oSettings.aoColumns, function ( i, col ) {
			if ( col.bVisible && jQuery(col.nTh).css('display') !== 'none' ) {
				vis++;
			}
		} );
	...
	function _fnFeatureHtmlLength ( settings )
	{
		...
		jQuery(settings.nTable).on( 'length.dt.DT', function (e, s, len) {
			if ( settings === s ) {
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		else
		{
			...
			jQuery('[name]', tmpTable).removeAttr('name');
		...
		if ( (tableWidthAttr || scrollX) && ! oSettings._reszEvt ) {
			var bindResize = function () {
				jQuery(window).on('resize.DT-'+oSettings.sInstance, _fnThrottle( function () {
	...
	function _fnBindAction( n, oData, fn )
	{
		jQuery(n)
			.on( 'click.DT', oData, function (e) {
					jQuery(n).blur(); // Remove focus outline for mouse users
					fn(e);
				} )
		...
		var run = function ( sel ) {
			...
			if ( sel.nodeName ) {
				...
				if ( rowIdx !== undefined ) {
					...
					var host = jQuery(sel).closest('*[data-dt-row]');
					return host.length ?
						[ host.data('dt-row') ] :
						[];
				}
	...
	var __column_selector = function ( settings, selector, opts )
	{
		...
		var run = function ( s ) {
			...
			var host = jQuery(s).closest('*[data-dt-column]');
			return host.length ?
				[ host.data('dt-column') ] :
				[];
		};
		...
		if ( vis !== undefined ) {
			this.iterator( 'table', function ( settings ) {
				...
				if ( ! settings.aiDisplay.length ) {
					jQuery(settings.nTBody).find('td[colspan]').attr('colspan', _fnVisbleColumns(settings));
				}
	...
	var __cell_selector = function ( settings, selector, opts )
	{
		...
		var run = function ( s ) {
			...
			// data attributes
			host = jQuery(s).closest('*[data-dt-row]');
			return host.length ?
				[ {
	...
	_api_register( 'destroy()', function ( remove ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			jQuery(window).off('.DT-'+settings.sInstance);
