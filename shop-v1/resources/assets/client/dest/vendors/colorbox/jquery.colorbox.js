...
(function ($, document, window) {
	...
	function makeSettings() {
		...
		settings.rel = settings.rel || element.rel || jQuery(element).data('rel') || 'nofollow';
		...
		if (!closing) {
			...
			if (settings.rel !== 'nofollow') {
				$related = jQuery('.' + boxElement).filter(function () {
					...
					if (data) {
						relRelated =  jQuery(this).data('rel') || data.rel || this.rel;
					}
	...
	function load () {
		...
		if (settings.inline) {
			...
			$inline = $tag(div).hide().insertBefore(jQuery(href)[0]);
			...
			.one('load', function () {
				...
				$.each(['alt', 'longdesc', 'aria-describedby'], function(i,val){
					var attr = jQuery(element).attr(val) || jQuery(element).attr('data-'+val);
					if (attr) {
			...
			$loadingBay.load(href, settings.data, function (data, status) {
				if (request === requests) {
					prep(status === 'error' ? $tag(div, 'Error').html(settings.xhrError) : jQuery(this).contents());
				}
			});
		}
	}
	...
	publicMethod.remove = function () {
		...
		jQuery(document).unbind('click.'+prefix);
	};
