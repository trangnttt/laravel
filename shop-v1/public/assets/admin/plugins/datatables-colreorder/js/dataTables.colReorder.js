...
$.fn.dataTableExt.oApi.fnColReorder = function ( oSettings, iFrom, iTo, drop, invalidateRows )
{
	...
	for ( i=0, iLen=iCols ; i<iLen ; i++ )
	{
		jQuery(oSettings.aoColumns[i].nTh).off('.DT');
		this.oApi._fnSortAttachListener( oSettings, oSettings.aoColumns[i].nTh, i );
	}


	/* Fire an event so other plug-ins can update */
	jQuery(oSettings.oInstance).trigger( 'column-reorder.dt', [ oSettings, {
		from: iFrom,
		to: iTo,
...
$.extend( ColReorder.prototype, {
	 ...
	 *    jQuery('#resetOrdering').click( function (e) {
	 *        e.preventDefault();
	...
	"_fnConstruct": function ()
	{
		...
		if ( aiOrder )
		{
			...
			{
				var bDone = false;
				jQuery(table).on( 'draw.dt.colReorder', function () {
		...
		// Destroy clean up
		jQuery(table).on( 'destroy.dt.colReorder', function () {
			jQuery(table).off( 'destroy.dt.colReorder draw.dt.colReorder' );

			$.each( that.s.dt.aoColumns, function (i, column) {
				jQuery(column.nTh).off('.ColReorder');
				jQuery(column.nTh).removeAttr('data-column-index');
			} );
	...
	"_fnMouseDown": function ( e, nTh )
	{
		...
		var target = jQuery(e.target).closest('th, td');
		var offset = target.offset();
		var idx = parseInt( jQuery(nTh).attr('data-column-index'), 10 );
	...
	"_fnMouseUp": function ( e )
	{
		...
		jQuery(document).off( '.ColReorder' );
	...
	"_fnRegions": function ()
	{
		...
		var firstColumnWidth = jQuery(aoColumns[firstColumn.index].nTh).outerWidth();
	...
	"_fnCreateDragNode": function ()
	{
		...
		var cloneCell = jQuery(origCell).clone();

		// This is a slightly odd combination of jQuery and DOM, but it is the
		// fastest and least resource intensive way I could think of cloning
		// the table with just a single header cell in it.
		this.dom.drag = jQuery(origTable.cloneNode(false))
			.addClass( 'DTCR_clonedTable' )
			.append(
				jQuery(origThead.cloneNode(false)).append(
					jQuery(origTr.cloneNode(false)).append(
						cloneCell[0]
					)
				)
			)
			.css( {
				...
				top: 0,
				left: 0,
				width: jQuery(origCell).outerWidth(),
				height: jQuery(origCell).outerHeight()
			} )
			.appendTo( 'body' );

		this.dom.pointer = jQuery('<div></div>')
			.addClass( 'DTCR_pointer' )
			.css( {
				position: 'absolute',
				top: scrolling ?
					jQuery('div.dataTables_scroll', this.s.dt.nTableWrapper).offset().top :
					jQuery(this.s.dt.nTable).offset().top,
				height : scrolling ?
					jQuery('div.dataTables_scroll', this.s.dt.nTableWrapper).height() :
					jQuery(this.s.dt.nTable).height()
			} )
			.appendTo( 'body' );
	},
	...
	"_fnSetColumnIndexes": function ()
	{
		$.each( this.s.dt.aoColumns, function (i, column) {
			jQuery(column.nTh).attr('data-column-index', i);
		} );
	},
    ...
    _fnIsLtr: function () {
        return jQuery(this.s.dt.nTable).css('direction') !== "rtl";
    }
} );
...
jQuery(document).on( 'preInit.dt.colReorder', function (e, settings) {
	if ( e.namespace !== 'dt' ) {
		return;
	}
